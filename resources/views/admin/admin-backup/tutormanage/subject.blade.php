    ...
    <div class="table-wrap">
      ...
      <div class="table-responsive">
        <table class="table theme-table">
          ...
          <tbody>
            <?php if(isset($subjectAll) && !empty($subjectAll)){ ?> 
            @php
            $i = 1; 
            @endphp
            @foreach($subjectAll as $data)
            <tr>
              <td>{{$i}}</td>
              <td>{{$data-> subject}}</td>
              <td>
                ...
                <div class="profile-dropdown">
                  <div class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                      ...
                      <li><a href="{{ url('admin/subject/delete_subject/'.$data->id) }}" onclick="return confirm('Are you sure?')">Delete</a></li>
                    </ul>
                  </div>
                </div>
              </td>
        ...
        <tfoot>
          <tr>
            <th colspan="9">   
              <?php 
              echo $subjectAll->appends($params)->render("pagination::bootstrap-4")  ;
              ?> 
            </th> 
          </tr>
        </tfoot>
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="fw-500">Subject Setup</h5>
        <div class="btn-close" data-bs-dismiss="modal" aria-label="Close"></div>
      </div>
      <form method="post" action="{{url('admin/subject/create_subject')}}">
        @csrf
...
<div class="modal theme-modal fade" id="modal-3">
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="fw-500">Edit Subject</h5>
        <div class="btn-close" data-bs-dismiss="modal" aria-label="Close"></div>
      </div>
      <form method="post" action="{{url('admin/subject/update_subject')}}">
        @csrf
        <div class="modal-body ps-5 pe-5">
          <input type="hidden" name="id" value="id" class="data_id" id="data_check_update_id">
          <div class="inp-outer">
            <label for="">Subject</label>
            <input class="input" id="data_check_update" type="text" name="subject" placeholder=""  >
          </div>
          <div class="modal-btn-group">
            <button class="site-link sm" data-bs-dismiss="modal" type="submit">Save</button>
          </div>
  ...
  $('document').ready(function(){
    $('.edit').on('click', function(event) {
     ...
     $.ajax({
      ...
      type : 'POST',
      url : "{{ url('/admin/subject/get_subject_value') }}",
      data : {'id': post_id},
      
      dataType : 'json',
      success : function(result){ 
        $('#data_check_update').val(result.subject);
